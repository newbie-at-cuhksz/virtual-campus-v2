...
public class WebServerClient : MonoBehaviour
{
        ...
        if (request.isDone)
        {
            ...
            else if (type == "downloadUniqueModel")
            {
                if (request.responseCode == 200)
                {
                    ...
                    FileInfo fileInfo = new FileInfo("./Assets/Resources/Images/" + request.GetResponseHeader("Content-Disposition"));
    ...
    {
        string url = baseURL + "image/upload/";
        byte[] image = File.ReadAllBytes(filePath+fileName);
        WWWForm form = new WWWForm();
        form.AddField("UID", UID);
        form.AddField("name", fileName);
        form.AddBinaryData("img",image);

        StartCoroutine(Post(form, url, "uploadImage"));
    }

    public void DownloadImage(string fileName)
    {
        //string fileName = "chair_test.obj";
        string url = baseURL + "download/image/";
        WWWForm form = new WWWForm();
        form.AddField("UID", UID);
        form.AddField("name", fileName);
        StartCoroutine(Post(form, url, "downloadImage"));
    }
